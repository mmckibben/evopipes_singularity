...
system ("makeblastdb -in $(pwd)/$PROT -dbtype prot");
...
LOOP1: while (<NAME>) {
	...
	if($load < 64) {
        ...
        system ("cd DupPipe/$taxon/; blastx -num_threads $CPU -evalue 0.01 -max_target_seqs 50 -db $(pwd)/$PROT -query unique_seqs.no_dups.list.no_recip_dups.no_dups.clean.parsed.duppairs.out.allvsall.$taxon -out out.blastx_$taxon");
        ...
        system ("cd DupPipe/$taxon/; prot_fasta2.pl prot_ids1.clean.no_te.unique_col0.clean.blastxparsed.out.blastx_$taxon $(pwd)/$PROT");

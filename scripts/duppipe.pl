...
# September 2020
#
# To run DupPipe from the docker: put the datafile(s) to run, a protein file for translations, and a file with a list of their names wherever in your folder you like.
# To execute the pipeline, enter at the command line prompt: perl duppipe.pl <names_file> <protein_file> <CPU_number>
#
#################
...
system ("makeblastdb -in  $abs_path/$PROT -dbtype prot");
...
LOOP1: while (<NAME>) {
	chomp $_;
        ...
        system ("cd DupPipe/$taxon/; blastx -num_threads $CPU -evalue 0.01 -max_target_seqs 50 -db  $abs_path/$PROT -query unique_seqs.no_dups.list.no_recip_dups.no_dups.clean.parsed.duppairs.out.allvsall.$taxon -out out.blastx_$taxon");
        ...
        system ("cd DupPipe/$taxon/; prot_fasta2.pl prot_ids1.clean.no_te.unique_col0.clean.blastxparsed.out.blastx_$taxon  $abs_path/$PROT");
        ...
        system ("cd DupPipe/$taxon/; blastn -task dc-megablast -num_threads $CPU -template_length 16 -template_type coding_and_optimal -word_size 11 -perc_identity 30 -evalue 0.01 -db /bin/ath.fasta -query unique_seqs.no_dups.list.no_recip_dups.no_dups.clean.parsed.duppairs.out.allvsall.$taxon -out out.ath_annotation_$taxon");
